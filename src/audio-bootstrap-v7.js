...
// Longer dry-leaf windows avoid the clicky/chopped effect from very short cuts.
const WALK_CUES = Object.freeze([
  { offset: 0.12, duration: 0.44 },
  { offset: 1.02, duration: 0.46 },
  { offset: 1.86, duration: 0.44 },
  { offset: 2.76, duration: 0.46 },
  { offset: 3.62, duration: 0.44 },
  { offset: 4.52, duration: 0.46 },
]);
...
// Keep the slower walk cadence from v6. Running remains clearly faster.
const STEP_PROFILE = Object.freeze({
  walk: { spacing: 1.52, playbackRate: 0.94, gain: 0.54 },
...
function birdWeight(hour) {
  // Birds stay present through daylight instead of dropping to a faint layer.
...
async function fetchBuffer(name) {
  const response = await fetch(`${AUDIO_BASE}${FILES[name]}?v=7`, { cache: "force-cache" });
...
async function ensureAudio() {
  ...
  try {
    ...
    activeProfile = "-";
    button.textContent = "🔊 เสียง";
    toast("🔊 เสียงนกกลางวันคงที่ + ตัดเสียงแมลงป่ากลางวันแล้ว");
  } catch (error) {
    console.error("Liora audio v7 init failed", error);
...
function playStep(kind) {
  ...
  const gain = context.createGain();
  source.buffer = buffer;
  source.playbackRate.value = profile.playbackRate * (0.985 + Math.random() * 0.03);
  const now = context.currentTime;
  const peak = profile.gain * (0.96 + Math.random() * 0.08);
  gain.gain.setValueAtTime(0.0001, now);
  gain.gain.exponentialRampToValueAtTime(Math.max(0.001, peak), now + 0.018);
  gain.gain.setValueAtTime(Math.max(0.001, peak), now + Math.max(0.03, duration - 0.07));
...
function update(now) {
  ...
  if (debug) {
    ...
    debug.textContent = [
      `AUDIO V7 ${unlocked ? (muted ? "MUTED" : "SYNC") : (loading ? "LOADING" : "LOCKED")}`,
