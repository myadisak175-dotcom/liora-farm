...
export function createBuilderController({
  ...
  function emitContext() { onContextChange(state.context, state); }
  ...
  function snapValue(value) { return Math.round(value / gridSize) * gridSize; }
  ...
  function normalizeItem(item) {
    ...
    if (![normalized.x, normalized.z, normalized.rotation, normalized.scale].every(Number.isFinite)) return null;
    return normalized;
  }
  ...
  function edgeLimit() {
    return Number.isFinite(worldHalfSize) ? Math.max(0, worldHalfSize - edgePadding) : Infinity;
  }
  ...
  /** Terrain slope rules are strictly opt-in per asset. */
  function groundConflict(assetId, x, z, scale) {
    const asset = catalog[assetId];
    if (asset?.maxGroundSlope == null || typeof getGroundHeight !== "function") return null;
  ...
  function stackConflict(assetId, x, z, scale, ignoreId) {
    const asset = catalog[assetId];
    const minDistance = Math.max(0.2, radiusOf(asset, scale, "footprintRadius") * 0.9);
    for (const other of items) {
      if (other.id === ignoreId || inactiveItemIds.has(other.id) || other.assetId !== assetId) continue;
      if (Math.hypot(x - other.x, z - other.z) < minDistance) return { other, minDistance };
    }
    return null;
  }
  ...
  function validatePlacement(assetId, transform, ignoreId = null) {
    ...
    if (![x, z, scale].every(Number.isFinite)) return { ok: false, code: "invalid-transform" };
    if (scale < asset.minScale - 1e-6 || scale > asset.maxScale + 1e-6) return { ok: false, code: "scale" };

    const footprint = radiusOf(asset, scale, "footprintRadius");
    const limit = edgeLimit();
    if (Math.abs(x) + footprint > limit || Math.abs(z) + footprint > limit) return { ok: false, code: "edge", radius: footprint };

    const reserved = reservedConflict(assetId, x, z, scale);
    if (reserved) return { ok: false, code: "reserved", label: reserved.other.label };

    const ground = groundConflict(assetId, x, z, scale);
    if (ground) return { ok: false, code: "slope", slope: ground.slope, maxSlope: ground.maxSlope };

    const conflict = blockingConflict(assetId, x, z, scale, ignoreId);
    if (conflict) return { ok: false, code: "collision", conflictId: conflict.other.id, conflictAssetId: conflict.other.assetId };

    const stacked = stackConflict(assetId, x, z, scale, ignoreId);
    if (stacked) return { ok: false, code: "stacked", conflictId: stacked.other.id };
  ...
  function resolvePlacement(assetId, transform, ignoreId = null) {
    ...
    for (let pass = 0; pass < 8; pass += 1) {
      const conflict = blockingConflict(assetId, x, z, scale, ignoreId) ?? reservedConflict(assetId, x, z, scale) ?? stackConflict(assetId, x, z, scale, ignoreId);
      if (!conflict) break;
  ...
  function setInactiveItems(ids = []) {
    ...
    for (const id of ids) if (items.some((item) => item.id === id)) inactiveItemIds.add(id);
  ...
  function enable() { state.enabled = true; enterIdleContext(state); emitContext(); }
  function disable({ saveLayout = true } = {}) {
    if (saveLayout) flushSave();
    state.enabled = false; enterIdleContext(state); onSelectionChange(null); onPreviewChange(null); emitContext();
  }
  function beginPlacement(assetId, { transform = null } = {}) {
    const asset = catalog[assetId];
    if (!asset) throw new Error(`Unknown buildable asset: ${assetId}`);
    enterPlaceContext(state, assetId); onPreviewChange({ assetId, asset, transform }); emitContext();
  }
  function cancelPlacement() { enterIdleContext(state); onPreviewChange(null); emitContext(); }

  function addItem(item, { validate = true, saveLayout = true, recordHistory = true } = {}) {
    const normalized = normalizeItem(item);
    if (!normalized) return null;
    if (validate && !validatePlacement(normalized.assetId, normalized).ok) return null;
    inactiveItemIds.delete(normalized.id); items.push(normalized);
    if (recordHistory) pushBuilderHistory(state, { type: "add", item: { ...normalized } });
  ...
  function selectItem(id) {
    ...
    if (!item) { enterIdleContext(state); onSelectionChange(null); emitContext(); return null; }
    enterEditContext(state, id); onSelectionChange(item); emitContext(); return item;
  }
  function clearSelection() { flushSave(); enterIdleContext(state); onSelectionChange(null); emitContext(); }
  ...
  function updateSelected(transform, { recordHistory = false } = {}) {
    ...
    const needsPlacementCheck = Object.prototype.hasOwnProperty.call(transform, "x") || Object.prototype.hasOwnProperty.call(transform, "z") || Object.prototype.hasOwnProperty.call(transform, "scale");
    if (needsPlacementCheck && !validatePlacement(item.assetId, next, item.id).ok) return null;
    if (recordHistory) pushBuilderHistory(state, { type: "transform", item: { ...item } });
    Object.assign(item, transform); save(); onSelectionChange(item); return item;
  }
  ...
  function duplicateSelected() {
    ...
    for (const ring of [1, 1.35, 1.7]) {
      for (const [dx, dz] of directions) {
        ...
        const candidate = { ...item, id: crypto.randomUUID(), x: item.x + (dx / length) * baseDistance * ring, z: item.z + (dz / length) * baseDistance * ring };
        if (validatePlacement(candidate.assetId, candidate).ok) return addItem(candidate);
      }
    }
    return null;
  }
  ...
  function deleteSelected() {
    ...
    const [removed] = items.splice(index, 1); inactiveItemIds.delete(removed.id);
    pushBuilderHistory(state, { type: "delete", item: { ...removed } });
    enterIdleContext(state); onSelectionChange(null); save({ immediate: true }); emitContext(); return removed;
  }
  ...
  function undo() {
    ...
    if (action.type === "add") {
      ...
      if (index < 0) return null;
      items.splice(index, 1); inactiveItemIds.delete(action.item.id); enterIdleContext(state); onSelectionChange(null); save({ immediate: true }); emitContext();
      return { type: "removed", item: action.item };
    }
    if (action.type === "delete") {
      inactiveItemIds.delete(action.item.id); items.push({ ...action.item }); enterIdleContext(state); onSelectionChange(null); save({ immediate: true }); emitContext();
      return { type: "restored", item: action.item };
    }
    if (action.type === "transform") {
      const item = items.find((entry) => entry.id === action.item.id);
      if (!item) return null;
      Object.assign(item, action.item); save({ immediate: true }); emitContext(); return { type: "moved", item };
    }
    return null;
  }
  ...
  function load() {
    ...
    for (const entry of raw) {
      ...
      if (normalized) usable.push(normalized); else if (entry?.assetId) unknownAssetIds.add(entry.assetId);
    }
    loadReport = { total: raw.length, dropped: raw.length - usable.length, unknownAssetIds: [...unknownAssetIds], storeIssue: layoutStore.lastIssue ?? null };
    inactiveItemIds.clear(); items.splice(0, items.length, ...usable); state.history.length = 0;
    enterIdleContext(state); onSelectionChange(null); onPreviewChange(null); onLayoutChange(items, null); emitContext();
    return items;
  }

  function resetTo(objects = []) {
    inactiveItemIds.clear(); items.splice(0, items.length);
    for (const object of objects) { const normalized = normalizeItem(object); if (normalized) items.push(normalized); }
    state.history.length = 0; enterIdleContext(state); onSelectionChange(null); onPreviewChange(null); save({ immediate: true }); onItemsRestored(items); emitContext(); return items;
  }

  return {
    items, enable, disable, save, flushSave, load, resetTo, undo, snapTransform,
    validatePlacement, resolvePlacement, setInactiveItems, getColliders,
    beginPlacement, cancelPlacement, addItem, selectItem, clearSelection,
    updateSelected, duplicateSelected, deleteSelected,
    get loadReport() { return loadReport; },
    get canUndo() { return state.history.length > 0; },
    get context() { return state.context; },
    get isPlacing() { return state.context === BUILDER_CONTEXTS.PLACE; },
    get isEditing() { return state.context === BUILDER_CONTEXTS.EDIT; },
  };
}
