...
export function createGroundPaint({ scene, camera, renderer, orbit, mount = document.body } = {}) {
  ...
  // The Hybrid Builder owns one large horizontal ground plane. Find it without
  // coupling this module to builder/index.html internals.
  ...
  function setMode(nextMode) {
    if (nextMode !== "off" && !MODE_TO_LAYER.hasOwnProperty(nextMode)) return;
    mode = nextMode;
    painting = false;
  ...
  // Initialize asynchronously while keeping the Builder-facing API synchronous.
  (async () => {
    try {
