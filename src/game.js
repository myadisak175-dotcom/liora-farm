...
function frame(){ ctx.setTransform(1,0,0,1,0,0); ctx.fillStyle="#0e1410"; ctx.fillRect(0,0,canvas.width,canvas.height); ctx.imageSmoothingEnabled=true; ctx.imageSmoothingQuality="high"; camera.apply(ctx); drawGround(ctx,assets,camera.view); if(showExits)drawExits(ctx); requestAnimationFrame(frame); }
...
async function start(){ const status=document.getElementById("status"); try{assets=await loadAssets()}catch(err){status.textContent=err.message;return} status.remove(); resize(); centreOnZone(); buildZoneBar(); requestAnimationFrame(frame); }
start();
