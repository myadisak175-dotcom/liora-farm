...
export const toolSelector = (() => {
  function getLayout() {
    const width = Math.min(148, Math.max(116, window.innerWidth * 0.37));
    const height = 42;
    return {
  ...
  function draw(ctx) {
    ...
    const resource = toolSystem.getResource(selected.id);
    const suffix = resource ? ` ${resource.amount}/${resource.capacity}` : "";
    ctx.fillStyle = "#ffffff";
    ctx.font = "700 14px system-ui, sans-serif";
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText(
      `${selected.icon} ${selected.name}${suffix}  ›`,
